...
Vous souhaitez concevoir une application web/mobile destinée aux amateurs de bières artisanales.

L'application devra permettre de découvrir des bières, consulter des informations sur leur origine, leurs caractéristiques, leurs ingrédients, ainsi que partager des avis et conserver des préférences.

Avant de développer l'application, il est nécessaire de concevoir une base de données relationnelle cohérente.
...
- contraintes ;
- règles d’intégrité ;
- comportement des suppressions si nécessaire.
...
Les propriétés proposées ci-dessous sont volontairement incomplètes.

Vous devez compléter et justifier votre modèle.
...
L'objectif du projet n'est pas d'apprendre Docker en profondeur.

Une configuration `docker-compose.yml` sera fournie.

---

## Travail attendu avec Docker

Vous devrez uniquement être capables de :

- lancer l’environnement fourni ;
- vérifier que PostgreSQL fonctionne ;
- vous connecter à la base avec DBeaver ;
- exécuter vos scripts SQL.
...
Si l’utilisateur a déjà noté cette bière, la note est mise à jour.
